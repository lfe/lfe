...
%%% them first. For the other forms we keep their order and output
%%% them in that order. This will result in somethings coming in the
%%% way that Erlang expects like keeping 'doc' and 'spec' before the
%%% functions they refer to.
%%%
%%% However it also means that there can come attributes in between
...
%% compile_form(Form, Forms, State) -> {Predefs,Rest,State}.
...
%% where we would like to check the immediately following form.
...
%% The predefs which must come first in the module before any function
%% definitions and other attributes. The module, exports and imports
%% will be added later when we have enough information.
...
compile_form(['compile',Line,Options], _Forms, St) ->
    {[make_attribute(compile, Options, Line)],[],St};
...
%% Normal attributes and functions which it is up to the user to get
%% in the right place.
