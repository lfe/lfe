...
-export([run_file/2,read_script_file/1,read_script_string/1,run_loop/2,
         slurp_errors/1]).
...
start(State) ->
    Repl = spawn(?MODULE, init, [State]),
    {ok,Repl}.

start_link(State) ->
    Repl = spawn_link(?MODULE, init, [State]),
    {ok,Repl}.

%% eval_form(EvalProcess, Form) ->
%%     {ok, Value, State} |
...
eval_form(Eval, Form) ->
    call(Eval, {eval_form, Form}).

get_state(Eval) ->
    call(Eval, get_state).

set_state(Eval, State) ->
    call(Eval, {set_state, State}).

%% call(EvalProcess, Call) -> Reply.
%% reply(Caller, Tag, Reply) -> ok.
%%  Send calls to evaluator and get reply in a safe way. We use the
%%  same way behaviour calls.

call(Eval, Call) ->
    Mref = erlang:monitor(process, Eval),
    From = self(),
    erlang:send(Eval, {eval_call, From, Mref, Call}, [noconnect]),
    receive
        {Mref,Reply} ->
            Reply;
...
%% init(State) -> no_return().
%%  Initialize the evaluator and call the loop.

init(State) ->
    eval_loop(State).

%% eval_loop(State) -> no_return().
%%  We currently ignore unrecognised messages.

eval_loop(State0) ->
    receive
        {eval_call,From,Tag,{eval_form,Form}} ->
            {Reply,State1} = eval_form(From, Tag, Form, State0),
            reply(From, Tag, Reply),
            eval_loop(State1);
        {eval_call,From,Tag,get_state} ->
            reply(From, Tag, {ok,State0}),
            eval_loop(State0);
        {eval_call,From,Tag,{set_state,StateNew}} ->
            reply(From, Tag, ok),
            eval_loop(StateNew)
    end.

%% eval_form(Form, Tag, Form, State) -> {ok,Value,State}.
%%  If the evaluation fails we do send exit signals to all linked
%%  processes except the requester to which we send a reply back then
%%  terminate the process.

eval_form(From, Tag, Form, State0) ->
    try
        ...
        ?CATCH(Class, Reason, StackTrace)
            ...
            lists:foreach(fun(P) -> exit(P, ER) end, LPs -- [From]),
            %% io:format("Out ~p\n", [{Class,Reason,StackTrace}]),
            reply(From, Tag, {Class,Reason,StackTrace}),
...
do_eval_form_1(['extend-module'|_], St) ->      %Maybe from macro expansion
    {[],St};
do_eval_form_1(['eval-when-compile'|_], St) ->
    %% Maybe from macro expansion, we can happily ignore this.
    {[],St};
...
%%  Read a file returning only the first sexpr for now.
%%  The same as lfe_io:read_string for now.
...
run_loop([F|Fs], _, St0) ->
    ...
    {Value,St1} = do_eval_form(F, St0#state{curr=Ce1}),
...
add_shell_functions(Env0) ->
    Fs = [
          {cd,1,[lambda,[d],[':',lfe_repl_cmds,cd,d]]},
          {ep,1,[lambda,[e],[':',lfe_repl_cmds,ep,e]]},
          {ep,2,[lambda,[e,d],[':',lfe_repl_cmds,ep,e,d]]},
          {epp,1,[lambda,[e],[':',lfe_repl_cmds,epp,e]]},
          {epp,2,[lambda,[e,d],[':',lfe_repl_cmds,epp,e,d]]},
          {h,0,[lambda,[],[':',lfe_repl_cmds,help]]},
          {h,1,[lambda,[m], [':',lfe_repl_cmds,h,m]]},
          {h,2,[lambda,[m,f], [':',lfe_repl_cmds,h,m,f]]},
          {h,3,[lambda,[m,f,a], [':',lfe_repl_cmds,h,m,f,a]]},

          {help,0,[lambda,[],[':',lfe_repl_cmds,help]]},
          {i,0,[lambda,[],[':',lfe_repl_cmds,i]]},
          {i,1,[lambda,[ps],[':',lfe_repl_cmds,i,ps]]},
          {i,3,[lambda,[x,y,z],[':',lfe_repl_cmds,i,x,y,z]]},
          {clear,0,[lambda,[],[':',lfe_repl_cmds,clear]]},
          {pid,3,[lambda,[i,j,k],[':',lfe_repl_cmds,pid,i,j,k]]},
          {p,1,[lambda,[e],[':',lfe_repl_cmds,p,e]]},
          {p,2,[lambda,[e,d],[':',lfe_repl_cmds,p,e,d]]},
          {pp,1,[lambda,[e],[':',lfe_repl_cmds,pp,e]]},
          {pp,2,[lambda,[e,d],[':',lfe_repl_cmds,pp,e,d]]},
          {pwd,0,[lambda,[],[':',lfe_repl_cmds,pwd]]},
          {q,0,[lambda,[],[':',lfe_repl_cmds,exit]]},
          {flush,0,[lambda,[],[':',lfe_repl_cmds,flush]]},
          {regs,0,[lambda,[],[':',lfe_repl_cmds,regs]]},
          {nregs,0,[lambda,[],[':',lfe_repl_cmds,nregs]]},
          {memory,0,[lambda,[],[':',lfe_repl_cmds,memory]]},
          {memory,1,[lambda,[t],[':',lfe_repl_cmds,memory,t]]},
          {uptime,0,[lambda,[],[':',lfe_repl_cmds,uptime]]},
          {exit,0,[lambda,[],[':',lfe_repl_cmds,exit]]}
         ],
...
add_shell_macros(Env0) ->
    ...
    %% expanded form here.
    Ms = [{c,[lambda,[args,'$ENV'],?BQ([':',lfe_repl_cmds,c,?C_A(args)])]},
          {describe,['match-lambda',
                     [[[list,mod],'$ENV'],
                      ?BQ([':',lfe_repl_cmds,doc,?Q(?C(mod))])],
                     ?UNDEF_MATCH_FUNC(describe)]},
          {doc,['match-lambda',
                [[[list,mod],'$ENV'],?BQ([':',lfe_repl_cmds,h,?Q(?C(mod))])],
                [[[list,mod,func],'$ENV'],
                 ?BQ([':',lfe_repl_cmds,h,?Q(?C(mod)),?Q(?C(func))])],
                [[[list,mod,func,arity],'$ENV'],
                 ?BQ([':',lfe_repl_cmds,h,?Q(?C(mod)),?Q(?C(func)),?Q(?C(arity))])],
                ?UNDEF_MATCH_FUNC(doc)]},
          {ec,[lambda,[args,'$ENV'],?BQ([':',lfe_repl_cmds,ec,?C_A(args)])]},
          {l,[lambda,[args,'$ENV'],?BQ([':',lfe_repl_cmds,l,[list|?C(args)]])]},
          {ls,[lambda,[args,'$ENV'],?BQ([':',lfe_repl_cmds,ls,[list|?C(args)]])]},
          {m,['match-lambda',
              [[[],'$ENV'],?BQ([':',lfe_repl_cmds,m])],
              [[ms,'$ENV'],?BQ([':',lfe_repl_cmds,m,[list|?C(ms)]])]]}
         ],
    %% Any errors here will crash shell startup!
