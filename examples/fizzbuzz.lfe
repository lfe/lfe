...
(export ((buzz 1) (buzz1 1) (buzz2 1) (buzz3 1)))
...
(function buzz3
  (lambda (col)
    ;; A tail-recursive implementation of FizzBuzz using a common
    ;; pattern within Erlang. Supplying an accmulator to keep our
    ;; recursion in a constant space.  The problem with this however
    ;; is that the list we create using this method will be reversed,
    ;; as Erlang only allows to append to the head of a list.
    (tail-buzz [] col)))
...
(function tail-buzz
  ...
  (match-lambda
    ([acc []]
     ;; Use the built in lists:reverse/1 function as it is is a much
     ;; loved component of Erlang and due to Erlang favouring this
     ;; form of tail recursive functions it has been highly optimised
     ;; for the task. There is a great explanation and discussion of
     ;; this topic in Learn You Some Erlang. I recommend you check it
     ;; out.
     (: lists reverse acc))
    ;; Similar to our other recursive implementation we split the list
    ;; we receive into a Head component (x) and a Tail component
    ;; (xs). In Erlang speak [H|T].  Note the lack of square
    ;; parenthesis, as we have two function arguments we need to wrap
    ;; them both in a list and simply a call to 'cons' is enough to
