    ...
    (let ((db (ets:new 'ets-demo '(#(keypos 2) set))))
      (let ((people
        ...
        (lists:foreach
         (match-lambda
           ([(tuple n p j)]
            (ets:insert db (record person name n place p job j))))
         people))
      db)))                             ;Return the table
...
(function by_place
  (lambda (db place)
    (let ((s1 (ets:match db (record person name '$1 place place job '$2)))
          (s2 (ets:match_object db (record person name '_ place place job '_))))
      (tuple s1 s2))))

;; Use match specifications to match records
(function by_place_ms
  (lambda (db place)
    (ets:select db (match-spec ([(record person name n place p job j)]
                                (when (=:= place p))
                                (list 'p n j))))))

(function not_painter
  (lambda (db place)
    (ets:select db (match-spec ([(record person name n place p job j)]
